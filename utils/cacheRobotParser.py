...
__all__ = ["CacheRobotFileParser"]

RequestRate = collections.namedtuple("RequestRate", "requests seconds")

class CacheRobotFileParser:
    """ This class provides a set of methods to read, parse and answer
    questions about a single robots.txt file.
    """

    def __init__(self, con, log,url=''):
    ...
    def read(self):
        ...
        self.resp_content = self.resp.raw_response.content.decode()

        if self.resp.raw_response.status_code in (401, 403):
            self.disallow_all = True
        elif self.resp.raw_response.status_code >= 400 and self.resp.raw_response.status_code < 500:
            self.allow_all = True
        self.parse(self.resp_content)
